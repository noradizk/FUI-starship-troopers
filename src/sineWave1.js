...
function resizeAll() {
  resizeCanvasToWrapper(canvas1);
  resizeCanvasToWrapper(canvas2);
  resizeCanvasToWrapper(canvas3);
  resizeCanvasToWrapper(canvas4);
...
function plotSine1(ctx1, t1){
    ...
    const w = ctx1.canvas.width;
    const h = ctx1.canvas.height;
    let x = 0;
    let y = 0;
    let amplitude = h / 2;
    let frequency = 20;
    ...
    while (x < w) {
        y = h / 2 + amplitude * Math.sin(x / frequency + t1);
        ctx1.lineTo(x,y);
        x = x + 1 ;
...
function plotSine2(ctx2, t2,){
    ...
    while (x < w) {
        y = h / 2 + amplitude * Math.sin(x / frequency + t2);
        ctx2.lineTo(x,y);
        x = x + 1;
...
function plotSine3(ctx3, t3) {
    ...
    const w = ctx3.canvas.width;
    const h = ctx3.canvas.height;
    let amplitude = h / 2;
    let frequency = 50;
    ...
    for (let i = 0; i < phases.length; i++) {
        ...
        // premier point (x = 0)
        let x = 0;
        let y = h / 2 + amplitude * Math.sin(x / frequency + t3 + phase);
        ctx3.moveTo(x, y);

        // on balaye x
        for (x = 1; x < w; x++) {
            y = h / 2 + amplitude * Math.sin(x / frequency + t3 + phase);
            ctx3.lineTo(x, y);
        }
...
/*     while(x<width){
    ...
    for(let i = 0; i< phases.length; i++){
        ...
        //premier point(x=0)
        let x = 0;
        let y = h / 2 + amplitude * Math.sin(x / frequency + t4 + phase);
        ctx4.moveTo(x, y);

         for (x = 1; x < w; x++) {
         y = h / 2 + amplitude * Math.sin(x / frequency + t4 + phase);
         ctx4.lineTo(x, y);
        }
      ctx4.stroke();
    }
